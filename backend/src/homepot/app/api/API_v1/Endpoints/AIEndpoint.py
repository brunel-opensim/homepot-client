...
class JobScheduleRequest(BaseModel):
    ...
    job_priority: str = Field(
        default="medium",
        description="Job priority: low, medium, high, critical"
    )
    earliest_start: Optional[str] = Field(
        None,
        description="Earliest acceptable start time (ISO format)"
    )
...
async def get_device_insights(
    ...
    try:
        ...
        # Get error analysis
        errors = await analytics.get_error_frequency_analysis(device_id, days=min(days, 7))
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate device insights: {str(e)}"
        )
...
async def get_site_insights(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate site insights: {str(e)}"
        )
...
async def predict_device_failure(
    ...
    window_hours: int = Query(
        ...
        ge=1,
        le=168,
        description="Prediction window in hours (max 7 days)"
    )
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to predict device failure: {str(e)}"
        )
...
async def get_at_risk_devices(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to identify at-risk devices: {str(e)}"
        )
...
async def calculate_success_probability(request: SuccessProbabilityRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to calculate success probability: {str(e)}"
        )
...
async def get_optimal_windows(
    ...
    day_of_week: Optional[int] = Query(
        ...
        ge=0,
        le=6,
        description="Optional day filter (0=Monday, 6=Sunday)"
    )
) -> Dict[str, Any]:
...
async def get_health_forecast(
    ...
    forecast_hours: int = Query(
        ...
        ge=1,
        le=168,
        description="Forecast period in hours"
    )
) -> Dict[str, Any]:
    ...
    try:
        ...
        at_risk = await predictor.identify_at_risk_devices(site_id, min_risk_level="low")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate health forecast: {str(e)}"
        )
