"""Canonical OS family classification and capability derivation.

Single source of truth for mapping an ``os_details`` string to:
- a canonical OS family key (``"windows"``, ``"android"``, ``"ios"``,
  ``"linux"``, ``"macos"``),
- the device capabilities the OS can support,
- the push-notification channel the OS would use.

Both the backend (``homepot.app.schemas.permissions``) and the POS emulator
(``emulators/pos_engine.py``) import these helpers so the two never drift
apart. The module is kept stdlib-only on purpose: the frozen emulator bundles
this exact file through PyInstaller, so it must not depend on pydantic or any
other backend dependency.
"""
...
def derive_push_channel(os_details: Optional[str]) -> Optional[str]:
    ...
    Mobile and push-capable OSes receive commands over a push transport (FCM
    on Android, WNS on Windows, APNs on iOS); all non-push OSes fall back to
    plain HTTP polling (``None``).
    """
    family = os_family(os_details)
