...
class PredictiveJobScheduler:
    ...
    async def recommend_execution_time(
        ...
        try:
            ...
            scheduling_windows = await self.analytics.get_optimal_scheduling_windows(site_id)
            ...
            if scheduling_windows.get("status") == "no_schedule":
                ...
                if job_priority == "critical":
                    ...
                    tomorrow_morning = (earliest_start + timedelta(days=1)).replace(hour=8, minute=0, second=0)
                    return {
            ...
            job_patterns = await self.analytics.get_job_outcome_patterns(site_id, days=30)
        ...
        try:
            ...
            if device_perf.get("status") != "error":
                ...
                factors.append({
                    ...
                    "weight": 0.4,
                    "score": health_score,
                    "impact": "positive" if health_score > 0.8 else "neutral" if health_score > 0.6 else "negative"
                })
            ...
            if scheduling.get("status") != "error":
                ...
                day_rec = next((r for r in recs if r["day_of_week"] == day_of_week), None)
            ...
            errors = await self.analytics.get_error_frequency_analysis(device_id, days=7)
    ...
    def _get_alternative_times(
        self,
        day_recommendation: Dict[str, Any],
    ...
    async def _find_next_open_slot(
        ...
        for offset in range(7):
            ...
            day_rec = next(
                (r for r in recommendations if r["day_of_week"] == check_day),
                None
            )
            
            if day_rec and day_rec["status"] == "open" and not day_rec.get("is_maintenance_window"):
                ...
                return {
                    ...
                    "site_status": day_rec,
                    "alternative_times": self._get_alternative_times(day_rec, target_date)
                }
