        ...
        try:
            ...
            async with db_service.get_session() as session:
                ...
                query = select(JobOutcome).where(
                    JobOutcome.completed_at >= cutoff_date
                )
                if site_id:
                ...
                if not outcomes:
                    return {
                        "status": "no_data",
                        "message": "No job outcomes available"
                    }
        ...
        try:
            ...
            async with db_service.get_session() as session:
                ...
                if day_of_week is not None:
                    query = query.where(SiteOperatingSchedule.day_of_week == day_of_week)
                
                result = await session.execute(query.order_by(SiteOperatingSchedule.day_of_week))
                ...
                for schedule in schedules:
                    ...
                    peak_start = schedule.peak_hours_start.hour if schedule.peak_hours_start else 12
                    peak_end = schedule.peak_hours_end.hour if schedule.peak_hours_end else 14
                    ...
                    recommendations.append({
                        ...
                        "recommendation": "Ideal for deployment" if optimal_windows and not schedule.is_maintenance_window else "Avoid - maintenance or limited windows"
                    })
    ...
    async def get_error_frequency_analysis(
        ...
        try:
            ...
            async with db_service.get_session() as session:
                ...
                result = await session.execute(query.order_by(ErrorLog.timestamp.desc()))
    ...
    async def get_configuration_impact_analysis(
        ...
        try:
            ...
            async with db_service.get_session() as session:
                ...
                result = await session.execute(query.order_by(ConfigurationHistory.changed_at.desc()))
