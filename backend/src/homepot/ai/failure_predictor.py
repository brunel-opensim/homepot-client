...
class FailurePredictor:
    ...
    async def identify_at_risk_devices(
        ...
        try:
            ...
            async with db_service.get_session() as session:
                ...
                for device in devices:
                    ...
                    if prediction.get("failure_probability", 0) >= risk_threshold:
                        at_risk_devices.append({
                            ...
                            "top_risk_factors": prediction.get("risk_factors", [])[:3],
                            "recommendations": prediction.get("recommendations", [])[:2]
                        })
                ...
                # Summary statistics
                risk_distribution = {
                    "critical": sum(1 for d in at_risk_devices if d["risk_level"] == "critical"),
                    "high": sum(1 for d in at_risk_devices if d["risk_level"] == "high"),
                    "medium": sum(1 for d in at_risk_devices if d["risk_level"] == "medium"),
    ...
    async def _analyze_error_patterns(self, device_id: str) -> Dict[str, Any]:
        ...
        try:
            errors = await self.analytics.get_error_frequency_analysis(device_id, days=1)
    ...
    async def _analyze_state_stability(self, device_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with db_service.get_session() as session:
                ...
                error_states = [t for t in transitions if "error" in t.new_state.lower()]
                if error_states:
                    score += 0.3
                    factors.append({
                        "category": "stability",
