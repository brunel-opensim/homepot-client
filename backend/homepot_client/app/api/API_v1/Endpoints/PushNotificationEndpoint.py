...
class PushSubscription(BaseModel):
    ...
    platform: str = Field(..., description="Platform type (fcm, wns, apns, web_push, mqtt)")
    device_token: str = Field(..., description="Device token or subscription info (JSON for Web Push)")
    device_info: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional device information"
    )
    user_id: Optional[str] = Field(default=None, description="User ID associated with subscription")
...
class SendNotificationRequest(BaseModel):
    ...
    priority: Optional[str] = Field(default="normal", description="Notification priority (high, normal, low)")
...
class BulkNotificationRequest(BaseModel):
    ...
    device_tokens: List[str] = Field(..., description="List of device tokens", min_length=1, max_length=1000)
    title: str = Field(..., description="Notification title", max_length=200)
    body: str = Field(..., description="Notification body", max_length=500)
    data: Optional[Dict[str, Any]] = Field(default=None, description="Additional notification data")
...
async def get_push_provider(platform: str):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Invalid platform '{platform}'. Supported: fcm, wns, apns, web_push, mqtt"
        )
...
@router.post("/subscribe", status_code=status.HTTP_201_CREATED, tags=["Push Notifications"])
