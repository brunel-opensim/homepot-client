...
def test_google_login_url_generation():
    ...
    with patch("homepot.app.api.API_v1.Endpoints.UserRegisterEndpoint.GOOGLE_CLIENT_ID", "test-client-id"), \
         patch("homepot.app.api.API_v1.Endpoints.UserRegisterEndpoint.GOOGLE_REDIRECT_URI", "http://localhost:5173/auth/callback"):
...
@patch("homepot.app.api.API_v1.Endpoints.UserRegisterEndpoint.get_or_create_google_user")
def test_google_callback_endpoint_flow(mock_get_user, mock_verify, mock_exchange):
    ...
    mock_verify.return_value = {"email": "callback@example.com", "name": "Callback User"}
    ...
    with patch("homepot.app.api.API_v1.Endpoints.UserRegisterEndpoint.FRONTEND_URL", "http://localhost:5173"):
        response = client.get("/api/v1/auth/callback?code=fake-code", follow_redirects=False)
