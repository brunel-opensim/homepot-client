...
class PushNotificationSettings(BaseSettings):
    ...
    default_ttl: int = Field(
        default=300,
        description="Default TTL for push notifications in seconds"
    )
    max_payload_size: int = Field(
        default=4096,
        description="Maximum push notification payload size in bytes"
    )
    collapse_key_prefix: str = Field(
        default="homepot",
        description="Prefix for push notification collapse keys"
    )
...
class DeviceSettings(BaseSettings):
    ...
    device_offline_threshold: int = Field(
        default=300,
        description="Seconds before marking device as offline"
    )
    max_concurrent_jobs: int = Field(
        default=10,
        description="Maximum concurrent jobs per device"
    )
