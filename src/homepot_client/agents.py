...
from homepot_client.models import Device, DeviceStatus
...
class POSAgentSimulator:
    ...
    def __init__(self, device_id: str, device_name: str):
        ...
        self.response_time_ms = random.randint(100, 500)  # Random response time
        ...
        logger.info(f"POS Agent {device_id} ({device_name}) initialized")
    ...
    async def handle_push_notification(self, notification_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Agent {self.device_id} received push notification: {notification_data.get('action', 'unknown')}")
            ...
            logger.error(f"Agent {self.device_id} error handling push notification: {e}")
            return {
                "status": "error",
                "message": str(e),
                "device_id": self.device_id,
    ...
    async def _handle_config_update(self, config_url: str, config_version: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Agent {self.device_id} validating config version {config_version}")
            await asyncio.sleep(0.2)
            
            # Simulate validation failures (3% chance)
            if random.random() < 0.03:
                raise Exception("Configuration validation failed: Invalid payment gateway settings")
            ...
            if random.random() < 0.02:
                raise Exception("POS application failed to restart: Service dependency error")
    ...
    async def _run_health_check(self) -> Dict[str, Any]:
        ...
        health_data = {
            ...
            "last_restart": (datetime.utcnow() - timedelta(hours=random.randint(1, 48))).isoformat(),
...
class AgentManager:
    ...
    async def send_push_notification(self, device_id: str, notification_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
