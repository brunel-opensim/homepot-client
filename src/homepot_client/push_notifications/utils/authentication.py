...
class ServiceAccountAuthenticator(Authenticator):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not GOOGLE_AUTH_AVAILABLE:
            raise ImportError("google-auth library required for service account authentication")
        
        self.service_account_path = config.get("service_account_path")
        self.scopes = config.get("scopes", ["https://www.googleapis.com/auth/firebase.messaging"])
        
        if not self.service_account_path:
            raise ValueError("service_account_path required for ServiceAccountAuthenticator")
...
class OAuth2Authenticator(Authenticator):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not all([self.client_id, self.client_secret, self.token_url]):
            raise ValueError("client_id, client_secret, and token_url required for OAuth2")
