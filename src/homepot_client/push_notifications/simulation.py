...
class SimulationProvider(PushNotificationProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(f"Initialized simulation provider with {self.success_rate*100}% success rate")
    ...
    async def send_notification(
        ...
        delay = random.uniform(delay_min, delay_max) / 1000.0  # Convert to seconds
        await asyncio.sleep(delay)
        
        # Simulate various error scenarios
        if self.error_scenarios and random.random() > self.success_rate:
        ...
        try:
            ...
            if response and response.get("status") == "success":
                ...
                error_msg = response.get("message", "Agent not found") if response else "No agent response"
                return PushNotificationResult(
                    success=False,
    ...
    async def send_topic_notification(
        ...
        try:
            ...
            return PushNotificationResult(
                ...
                platform=self.platform_name,
                message_id=f"topic-sim-{random.randint(100000, 999999)}",
            )
    ...
    def _convert_to_agent_format(self, payload: PushNotificationPayload) -> Dict[str, Any]:
        ...
        if "config_url" in payload.data:
            ...
            notification["data"]["config_version"] = payload.data.get("config_version", "1.0.0")
            ...
            valid_actions = ["update_pos_payment_config", "restart_pos_app", "health_check"]
            if payload.data["action"] in valid_actions:
                notification["action"] = payload.data["action"]
        
        return notification
    
    def _simulate_error(self, device_token: str, payload: PushNotificationPayload) -> PushNotificationResult:
        """Simulate various error scenarios.
