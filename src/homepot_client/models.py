...
class Site(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    site_id = Column(String(50), unique=True, index=True, nullable=False)  # e.g., "site-123"
...
class Job(Base):
    ...
    # Targeting
    site_id = Column(Integer, ForeignKey("sites.id"), nullable=False)
    device_id = Column(Integer, ForeignKey("devices.id"), nullable=True)  # Null for site-wide jobs
...
class AuditLog(Base):
    ...
    # Event details
    event_type = Column(String(50), nullable=False)  # e.g., "job_created", "device_updated"
