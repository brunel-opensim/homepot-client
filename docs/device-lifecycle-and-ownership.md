...
| U3  | **Device permissions backend (DB model + API)** ✅ | Add `device_permissions` JSON column to `Device` model. Create `PATCH /devices/device/{device_id}/permissions` (device auth via X-Device-ID + X-API-Key) and `GET /devices/device/{device_id}/permissions` (operator JWT). Include permissions in `GET /devices/device/{id}` response. Schema: `{ root_access, process_monitoring, filesystem_access, network_monitoring }`, all default `false`. |
